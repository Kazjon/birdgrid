...
DIRECTORY_NAME=str(config['SPECIES-Name'])+"-"+str(config['START-YEAR'])+"-"+str(config['END-YEAR'])+"-"+str(config['GRID_SIZE'])+"-"+str(config['PREDICTOR'])
PICKLE_NAME=str(config['SPECIES-Name'])+"-"+str(config['START-YEAR'])+"-"+str(config['END-YEAR'])+"-"+str(config['GRID_SIZE'])+"-"+str(config['PREDICTOR'])
...
if os.path.isfile(PICKLE_NAME+".p"):
	...
	locations=init_birdgrid(observations,GRID_SIZE,SPECIES,TIME_STEP,START_YEAR,END_YEAR,PICKLE_NAME,config)  #Calculate these from the above, Array of dicts, each dict contains lat, lon and data for each timestep
...
for k,location in locations.groupby(['LATITUDE','LONGITUDE'],as_index=False):
	predictors.append(model_location_novelty_over_time(location,SPECIES,SEASONS,PREDICTION_START_YEAR,PREDICTION_YEAR,config))

plot_birds_over_time(predictors,SPECIES,locations,DIRECTORY_NAME,config,PREDICTION_START_YEAR,PREDICTION_YEAR)
