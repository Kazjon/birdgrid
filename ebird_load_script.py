...
metadata = [s for s in metadata if not any([s.startswith(x) for x in metadata_excluded_prefixes])]
...
def ebird_metadata_setup(collname, fields, species, insert_timefield = True):
...
if __name__ == "__main__":
	...
	missing_thresh = 0.75 #If fewer than this percentage of required attributes are present in an individual record, discard it.
	...
	ebird_metadata_setup(args.output, metadata, species)